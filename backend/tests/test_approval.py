...
async def test_auto_expire_on_list() -> None:
	session = FakeAsyncSession()
	approval = FakeApproval(status="pending", expires_at=datetime.now(tz=UTC) - timedelta(minutes=1))
	session.queue_scalars([approval])
...
async def test_approval_api_list() -> None:
	...
	try:
		resp = await http_get("/api/v1/admin/approvals?status=pending", headers=auth_header("finance"))
