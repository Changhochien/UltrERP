        ...
        <div className="flex items-center gap-3 rounded-2xl border border-sidebar-border/70 bg-sidebar-accent/55 px-3 py-3">
          ...
          {showLabel ? (
            <div className="min-w-0 space-y-1">
              ...
              <p className="text-xs text-sidebar-foreground/60">AI-native ERP for Taiwan SMBs</p>
            </div>
          ) : null}
        </div>
      </SidebarHeader>
                  ...
                  return (
                    <SidebarMenuItem key={item.to}>
                      <Tooltip>
                        ...
                        {!showLabel ? <TooltipContent side="right">{item.label}</TooltipContent> : null}
                      </Tooltip>
                    </SidebarMenuItem>
                  );
                })}
            ...
            >
              ...
              {showLabel ? (
                <div className="min-w-0 flex-1 space-y-1">
                  <p className="truncate text-sm font-medium">{user?.sub ?? "guest"}</p>
                  <div className="flex flex-wrap items-center gap-2">
                    <Badge variant="outline" className="border-sidebar-border/80 bg-transparent text-[10px] text-sidebar-foreground/60">
                      {user?.role ?? "guest"}
                    </Badge>
                    <span className="truncate text-xs text-sidebar-foreground/55">{user?.tenant_id ?? "No tenant"}</span>
                  </div>
                </div>
              ) : null}
            </button>
          ...
          <DropdownMenuContent side="top" align="end" className="w-60">
            ...
            <DropdownMenuItem asChild>
              <div className="flex items-center justify-between gap-3 px-0 py-0 focus:bg-transparent">
                <span className="px-2.5 py-2 text-sm">Theme</span>
                <ThemeToggle />
              </div>
            </DropdownMenuItem>
            <DropdownMenuSeparator />
