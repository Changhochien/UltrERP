...
async def create(data: PaymentCreate, session: DbSession) -> PaymentResponse | JSONResponse:
	try:
		...
		return JSONResponse(
			status_code=409,
			content=error_response([{"field": "", "message": "Concurrent conflict. Please retry."}]),
		)
...
async def create_unmatched(data: PaymentCreateUnmatched, session: DbSession) -> PaymentResponse | JSONResponse:
	try:
		...
		return JSONResponse(
			status_code=422,
			content=error_response([{"field": "customer_id", "message": "Invalid customer or concurrent conflict."}]),
		)
...
async def confirm_match(
	payment_id: uuid.UUID, session: DbSession,
