	...
	async with session.begin():
		...
		for idx, line_data in enumerate(data.lines, start=1):
			try:
				...
				raise HTTPException(status_code=422, detail=[
					{"field": f"lines[{idx - 1}].tax_policy_code", "message": f"Invalid tax policy code: {line_data.tax_policy_code}"},
				])
			...
			stock_stmt = (
				select(func.coalesce(func.sum(InventoryStock.quantity), 0))
				.where(
...
async def confirm_order(
	...
	async with session.begin():
		...
		try:
			invoice = await _create_invoice_core(session, invoice_data, tid, buyer_identifier_normalized)
