...
async def test_product_detail_multi_warehouse() -> None:
	...
	stock_a = FakeStockRow(
		warehouse_id=wh_a_id, warehouse_name="Alpha",
		quantity=50, reorder_point=20,
	)
	stock_b = FakeStockRow(
		warehouse_id=wh_b_id, warehouse_name="Beta",
		quantity=10, reorder_point=15,
	)
...
async def test_warehouse_stock_fields() -> None:
	product = FakeProduct()
	stock = FakeStockRow(
		warehouse_name="Main", quantity=50, reorder_point=20,
	)
...
async def test_adjustment_history_fields() -> None:
	product = FakeProduct()
	adj = FakeAdjustment(
		quantity_change=-5, reason_code="damaged",
		actor_id="user1", notes="Broken items",
	)
...
async def test_history_offset_query_param() -> None:
	...
	try:
		resp = await _get(
			f"/api/v1/inventory/products/{product.id}",
			history_limit=10, history_offset=5,
		)
		assert resp.status_code == 200
	finally:
		_teardown(prev)
