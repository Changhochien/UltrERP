...
@pytest.mark.anyio
async def test_invoice_detail_creates_persisted_egui_state_when_enabled(
...
@pytest.mark.anyio
async def test_invoice_egui_refresh_recovers_from_invalid_persisted_status(
    monkeypatch: pytest.MonkeyPatch,
):
...
async def test_invoice_list_unpaid_query_excludes_legacy_paid_status() -> None:
    ...
    statement = _find_invoice_page_statement(session)
    sql, params = _compiled_sql(statement)
    status_lists = [list(value) for value in params.values() if isinstance(value, (list, tuple))]
