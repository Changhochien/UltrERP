...
import { useARAging } from "../hooks/useDashboard";
...
interface ARAgingCardProps {
  data: ReturnType<typeof useARAging>["data"];
  isLoading: boolean;
  error: string | null;
  onRetry: () => void;
}
  ...
  return (
    <Card data-testid="ar-aging-card" className="h-full">
      ...
      <CardContent className="space-y-4 pt-0">
        ...
        <div data-testid="ar-aging-buckets" className="grid grid-cols-4 gap-2">
          {buckets.map((bucket: ARAgingBucket) => (
            <div
              key={bucket.bucket_label}
              data-testid={`ar-aging-bucket-${bucket.bucket_label}`}
