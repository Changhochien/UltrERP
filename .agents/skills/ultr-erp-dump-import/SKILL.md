...
9. If the operator wants this workflow moved under `erp-skills/`, explain that `erp-skills/` is not a default project-skill discovery path. The supported locations are `.github/skills/`, `.Codex/skills/`, and `.agents/skills/` unless `chat.skillsLocations` is configured explicitly.
