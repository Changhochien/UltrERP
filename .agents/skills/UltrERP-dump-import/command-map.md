...
cd backend && uv run python -m domains.legacy_import.cli extract \
  ...
  --output ../legacy-migration-pipeline/extracted_data/<batch-id> \
...
**Default parameter values (auto-derived, operator can override):**
- `--output` is relative to `backend/`, so `../legacy-migration-pipeline/extracted_data/<batch-id>` places files at the repo root under `legacy-migration-pipeline/extracted_data/<batch-id>/`
- `--format` defaults to `csv` — this is the only format the import pipeline consumes
...
cd backend && uv run python -m domains.legacy_import.cli stage \
  --batch-id <batch-id> \
  --source-dir ../legacy-migration-pipeline/extracted_data/<batch-id> \
  --schema raw_legacy \
  --tenant-id <tenant-uuid>
```

**Default parameter values (auto-derived, operator can override):**
- `--source-dir` is derived as `../legacy-migration-pipeline/extracted_data/<batch-id>` — the standard output location from the `extract` phase
