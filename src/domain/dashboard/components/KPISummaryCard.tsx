    ...
    <SectionCard title={t("dashboard.kpi.title")} description={t("dashboard.kpi.description")}>
      <div data-testid="kpi-card" className="space-y-2">
        <div className="flex justify-between items-center">
          <span className="text-sm text-muted-foreground">{t("dashboard.kpi.todayRevenue")}</span>
          <span className="font-semibold">{formatTWD(data.today_revenue)}</span>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-sm text-muted-foreground">{t("dashboard.kpi.openInvoices")}</span>
          <span className="font-semibold">{data.open_invoice_count}</span>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-sm text-muted-foreground">{t("dashboard.kpi.openInvoiceAmount")}</span>
          <span className="font-semibold">{formatTWD(data.open_invoice_amount)}</span>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-sm text-muted-foreground">{t("dashboard.kpi.pendingOrders")}</span>
          <span className="font-semibold">{data.pending_order_count}</span>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-sm text-muted-foreground">{t("dashboard.kpi.pendingOrderRevenue")}</span>
          <span className="font-semibold">{formatTWD(data.pending_order_revenue)}</span>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-sm text-muted-foreground">{t("dashboard.kpi.lowStock")}</span>
          <span className="font-semibold">{data.low_stock_product_count}</span>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-sm text-muted-foreground">{t("dashboard.kpi.overdueReceivables")}</span>
