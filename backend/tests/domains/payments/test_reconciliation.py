...
def _queue_reconciliation(
	...
	for i, payment in enumerate(unmatched_payments):
		...
		if len(exact_invoices) == 1:
			...
			# Try date proximity
			candidate = (date_candidates_per_payment or [])[i] if date_candidates_per_payment and i < len(date_candidates_per_payment) else None
...
async def test_record_unmatched_payment():
	...
	try:
		resp = await http_post("/api/v1/payments/unmatched", {
...
async def test_manual_match_success():
	...
	prev = setup_session(session)
	try:
		resp = await http_post(f"/api/v1/payments/{payment.id}/manual-match", {
