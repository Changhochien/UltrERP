...
async def _generate_payment_ref(
	...
	result = await session.execute(
		select(func.max(Payment.payment_ref)).where(
	...
	async with session.begin():
		...
		if invoice is None:
			raise ValidationError([
				{"field": "invoice_id", "message": "Invoice not found."}
			])

		# AC4: Voided invoice guard
		if invoice.status == "voided":
			raise ValidationError([
				{"field": "invoice_id", "message": "Cannot record payment against voided invoice."}
			])
...
async def run_reconciliation(
	...
	async with session.begin():
		...
		for payment in unmatched_payments:
			...
			if len(exact_matches) == 1:
				...
				await session.flush()
				matched.append({"payment_id": str(payment.id), "payment_ref": payment.payment_ref, "invoice_number": inv.invoice_number})
				continue
			...
			# No match found
			unmatched_list.append({"payment_id": str(payment.id), "payment_ref": payment.payment_ref})
	...
	return {
		...
		"details": [
			{**d, "match_status": "matched", "match_type": "exact_amount"} for d in matched
		] + [
...
async def _allocate_payment(
	...
	*,
	...
	if invoice.status != "issued":
		raise ValidationError([
			{"field": "invoice_id", "message": f"Cannot allocate payment: invoice status is '{invoice.status}'."}
		])
...
async def confirm_suggested_match(
	...
	async with session.begin():
		...
		if invoice is None:
			raise ValidationError([{"field": "invoice_id", "message": "Suggested invoice no longer exists."}])

		if invoice.status != "issued":
			if invoice.status == "voided":
				raise ValidationError([{"field": "invoice_id", "message": "Invoice has been voided."}])
			raise ValidationError([{"field": "invoice_id", "message": "Can only match to issued invoices."}])
		...
		if payment.amount > outstanding:
			raise ValidationError([
				{"field": "amount", "message": "Payment amount exceeds invoice outstanding balance."}
			])

		await _allocate_payment(session, payment, invoice, payment.match_type or "manual", tid, actor_id)
		await session.flush()
...
async def manual_match(
	...
	async with session.begin():
		...
		if payment.match_status == "matched":
			raise ValidationError([{"field": "payment_id", "message": "Payment is already matched."}])
		...
		await _allocate_payment(
			session, payment, invoice, "manual", tid, actor_id,
